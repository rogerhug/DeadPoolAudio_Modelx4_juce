...
{
public:
    mainMenuMix(AudioProcessorValueTreeState& vts);
    ...
    //***************************************compressor*******************
    TextButton CompInitButton;
    Label CompLabel;
    Slider CompThrsSliders[8];
    Slider CompRatioSliders[8];
    Slider CompKneeSliders[8];
    Slider CompAttackSliders[8];
    Slider CompReleaseSliders[8];
    Slider CompMakeUpSliders[8];
    char ecc[15] = { "" };
    const char* CompValueNames(int n, int e) { sprintf(ecc, "CompTvalue%.2i%.1i", n, e);  return ecc; }
    char Temp[200] = { "-" };
    void serialCompParas(AudioProcessorValueTreeState& valueTreeState, double v, int n, int p);
    void unSerialCompParas(AudioProcessorValueTreeState& valueTreeState, int s, int n);
    char* readOut() { return Temp; }
    //**********************************************************************
