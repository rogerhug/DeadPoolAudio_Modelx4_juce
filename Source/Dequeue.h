...
kQueues2::kQueues2 (int k1, int n1)
{
...
// To enqueue an item in queue number 'qn' where qn is from 0 to k-1
void kQueues2::enqueue(int item, int qn)
{
	// Overflow check
	if (isFull())
	{
...
// Constructor to create k queues in an array of size n
kQueues1::kQueues1(int k1, int n1)
{
	// Initialize n and k, and allocate memory for all arrays
	k = k1, n = n1;
	arr = new int[n];
...
// To enqueue an item in queue number 'qn' where qn is from 0 to k-1
void kQueues1::enqueue(int item, int qn)
{
	// Overflow check
	if (isFull())
	{
