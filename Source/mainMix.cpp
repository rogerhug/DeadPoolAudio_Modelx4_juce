...
mainMenu::mainMenu()
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        MainMenuButtons[i].setBounds(210 + i * 60, 2, 60, 20);
        ...
        MainMenuButtons[5].setButtonText("Comp");
        if (i == 0)
...
mainMenuMix::mainMenuMix(AudioProcessorValueTreeState& vts)
 
{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        addAndMakeVisible(CompThrsSliders[i]);

        CompThrsSliders[i].setSliderStyle(juce::Slider::LinearHorizontal);
        CompThrsSliders[i].setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::silver);
        CompThrsSliders[i].setTextBoxStyle(juce::Slider::TextBoxRight, false, 35, 20);
        CompThrsSliders[i].setBounds(0, 20 + i * buttonSethigh, 100, buttonSethigh);
        CompThrsSliders[i].setRange(0.0, 1.0, 0.01);
        CompThrsSliders[i].setVisible(false);
        CompThrsSliders[i].setLookAndFeel(mlaf);
       

        addAndMakeVisible(CompRatioSliders[i]);

        CompRatioSliders[i].setSliderStyle(juce::Slider::LinearHorizontal);
        CompRatioSliders[i].setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::silver);
        CompRatioSliders[i].setTextBoxStyle(juce::Slider::TextBoxRight, false, 35, 20);
        CompRatioSliders[i].setBounds(100, 20 + i * buttonSethigh, 100, buttonSethigh);
        CompRatioSliders[i].setRange(0.0, 1.0, 0.01);
        CompRatioSliders[i].setVisible(false);
        CompRatioSliders[i].setLookAndFeel(mlaf);
       

        addAndMakeVisible(CompKneeSliders[i]);

        CompKneeSliders[i].setSliderStyle(juce::Slider::LinearHorizontal);
        CompKneeSliders[i].setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::silver);
        CompKneeSliders[i].setTextBoxStyle(juce::Slider::TextBoxRight, false, 35, 20);
        CompKneeSliders[i].setBounds(200, 20 + i * buttonSethigh, 100, buttonSethigh);
        CompKneeSliders[i].setRange(0.0, 1.0, 0.01);
        CompKneeSliders[i].setValue(0.1, juce::dontSendNotification);
        CompKneeSliders[i].setVisible(false);
        CompKneeSliders[i].setLookAndFeel(mlaf);


        addAndMakeVisible(CompAttackSliders[i]);

        CompAttackSliders[i].setSliderStyle(juce::Slider::LinearHorizontal);
        CompAttackSliders[i].setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::silver);
        CompAttackSliders[i].setTextBoxStyle(juce::Slider::TextBoxRight, false, 35, 20);
        CompAttackSliders[i].setBounds(300, 20 + i * buttonSethigh, 100, buttonSethigh);
        CompAttackSliders[i].setRange(0.0, 4.0, 0.01);
        CompAttackSliders[i].setVisible(false);
        CompAttackSliders[i].setLookAndFeel(mlaf);

        addAndMakeVisible(CompReleaseSliders[i]);

        CompReleaseSliders[i].setSliderStyle(juce::Slider::LinearHorizontal);
        CompReleaseSliders[i].setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::silver);
        CompReleaseSliders[i].setTextBoxStyle(juce::Slider::TextBoxRight, false, 35, 20);
        CompReleaseSliders[i].setBounds(400, 20 + i * buttonSethigh, 100, buttonSethigh);
        CompReleaseSliders[i].setRange(0.0, 4.0, 0.01);
        CompReleaseSliders[i].setVisible(false);
        CompReleaseSliders[i].setLookAndFeel(mlaf);

        
        addAndMakeVisible(CompMakeUpSliders[i]);

        CompMakeUpSliders[i].setSliderStyle(juce::Slider::LinearHorizontal);
        CompMakeUpSliders[i].setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::silver);
        CompMakeUpSliders[i].setTextBoxStyle(juce::Slider::TextBoxRight, false, 35, 20);
        CompMakeUpSliders[i].setBounds(500, 20 + i * buttonSethigh, 100, buttonSethigh);
        CompMakeUpSliders[i].setRange(0.0, 1.0, 0.01);
        CompMakeUpSliders[i].setVisible(false);
        CompMakeUpSliders[i].setLookAndFeel(mlaf);

    }
    addAndMakeVisible(CompInitButton);
    CompInitButton.setBounds(610, 20, 20, 20);
    addAndMakeVisible(CompLabel);
    CompLabel.setBounds(0, 0, 650, 20);
    CompLabel.setText("threshold______Ratio__________Knee_________Att___________Rel____________MakeUp", juce::dontSendNotification);
...
void mainMenuMix::init()
{
    for (int i = 0; i < 7; i++)
    {
        CompThrsSliders[i].setValue(0.3, juce::dontSendNotification);
        CompRatioSliders[i].setValue(0.1, juce::dontSendNotification);
        CompKneeSliders[i].setValue(0.1, juce::dontSendNotification);
        CompAttackSliders[i].setValue(0.01, juce::dontSendNotification);
        CompReleaseSliders[i].setValue(0.8, juce::dontSendNotification);
        CompMakeUpSliders[i].setValue(0.5, juce::dontSendNotification);
...
void mainMenuMix::changeMode(int s)
{
    ...
    for (int v = 0; v < 16; v++)
    {
        StepperValueSliders[v].setValue(stepsValueIn[s][0][v], juce::dontSendNotification);
    }
}
              ...
              for (int v = 0; v < 16; v++)
                {
                    if (m == 0) {
                        ...
                        stepsValueIn[s][on[s]][v] = 0.5;// StepperValueSliders[v].getValue();
                  
                    }
                    
                    if (m == 1) {
                        StepperValueSliders[v].setValue(stepDnini[v], juce::dontSendNotification);
                        stepsValueIn[s][on[s]][v] = stepDnini[v];// StepperValueSliders[v].getValue();
                       
                    }
                    if (m == 2) {
                        StepperValueSliders[v].setValue(stepupini[v], juce::dontSendNotification);
                        stepsValueIn[s][on[s]][v] = stepupini[v];// StepperValueSliders[v].getValue();
                       
                    }
                    if (m == 3) {
                        StepperValueSliders[v].setValue(stepsinini[v], juce::dontSendNotification);
                        stepsValueIn[s][on[s]][v] = stepsinini[v];// StepperValueSliders[v].getValue();
...
void mainMenuMix::resetPatternSteps(int s)
{
    for (int e = 0; e < 8; e++)
    {
        for (int i = 0; i < 11; i++)
        {
            for (int v = 0; v < 16; v++)
            {
                stepsValueIn[s][i][v]= StepperValueSliders[v].getValue();
...
void mainMenuMix::serialStepps(AudioProcessorValueTreeState& valueTreeState ,double*v,int n,int p)
{
    for (int i = 0; i < 16; i++) {
        ...
        sprintf(serialStepValues[n][0], "%.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f"
            ...
            valueTreeState.state.setProperty(StepValuesNames(0,n), var(serialStepValues[n][0]), nullptr);

           
        }
   
    strcpy(crd, SteppValues[n][0].getValue().toString().toRawUTF8());
}

void mainMenuMix::unSerialStepps(AudioProcessorValueTreeState& valueTreeState,int s1,int p)
{
    
    for (int s = 0; s < 7; s++) {
        SteppValues[s][0].referTo(valueTreeState.state.getPropertyAsValue(StepValuesNames(0, s), nullptr, true));
        ...
        for (int y = 0; y <= 80; y += 5) {
            ...
            stepsValueIn[s][0][y / 5] = SteppValues[s][0].getValue().toString().substring(y, y + 4).getDoubleValue();
...
void mainMenuMix::changeStepperColor(int s,int p)
{
     ...
     for (int i = 0; i < 16; ++i) {
          ...
          // StepperValueSliders[i].setValue(stepsOut[s][0][i], juce::dontSendNotification);
           StepperValueSliders[i].setValue(stepsValueIn[s][0][i], juce::dontSendNotification);
    }
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        for (int m = 0; m < 8; ++m) {
            ...
            if (n == 0) {
               ...
               CompInitButton.setVisible(false);
               CompLabel.setVisible(false);
               CompThrsSliders[m].setVisible(false);
               CompRatioSliders[m].setVisible(false);
               CompKneeSliders[m].setVisible(false);
               CompAttackSliders[m].setVisible(false);
               CompReleaseSliders[m].setVisible(false);
               CompMakeUpSliders[m].setVisible(false);
            ...
            if (n == 5) {
                ...
                CompInitButton.setVisible(true);
                CompLabel.setVisible(true);
                CompThrsSliders[m].setVisible(true);
                CompRatioSliders[m].setVisible(true);
                CompKneeSliders[m].setVisible(true);
                CompAttackSliders[m].setVisible(true);
                CompReleaseSliders[m].setVisible(true);
                CompMakeUpSliders[m].setVisible(true);
...
double compParasIn[8][6];
char serialcompParasIn[8][30] = { 0 };
void mainMenuMix::serialCompParas(AudioProcessorValueTreeState& valueTreeState, double v, int n, int p)
{
   

        compParasIn[n][p] = v;
        sprintf(serialcompParasIn[n], "%.2f %.2f %.2f %.2f %.2f %.2f"
            , compParasIn[n][0], compParasIn[n][1], compParasIn[n][2], compParasIn[n][3], compParasIn[n][4], compParasIn[n][5]);


        valueTreeState.state.setProperty(CompValueNames(0, n), serialcompParasIn[n], nullptr);

        //strcpy(Temp, serialcompParasIn[n]);
    
}
void mainMenuMix::unSerialCompParas(AudioProcessorValueTreeState& valueTreeState, int h, int n)
{
    for (int s = 0; s < 8; s++) {
        for (int p = 0; p < 6; p++) {
            CompParasValue[s].referTo(valueTreeState.state.getPropertyAsValue(CompValueNames(0, s), nullptr, true));
            ...
            for (int y = 0; y <= 35; y += 5) {
                ...
                compParasIn[s][y / 5] = CompParasValue[s].getValue().toString().substring(y, y + 4).getDoubleValue();


            }

        }
      strcpy(Temp, CompParasValue[0].getValue().toString().toRawUTF8());
    
    CompThrsSliders[s].setValue(compParasIn[s][0], juce::dontSendNotification);
    CompRatioSliders[s].setValue(compParasIn[s][1], juce::dontSendNotification);
    CompKneeSliders[s].setValue(compParasIn[s][2], juce::dontSendNotification);
    CompAttackSliders[s].setValue(compParasIn[s][3], juce::dontSendNotification);
    CompReleaseSliders[s].setValue(compParasIn[s][4], juce::dontSendNotification);
    CompMakeUpSliders[s].setValue(compParasIn[s][5], juce::dontSendNotification);
    }
}
