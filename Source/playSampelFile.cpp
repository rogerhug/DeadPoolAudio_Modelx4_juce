...
void AudioSample::processAudioSampel(const int *trigger,double*out,int nFrames)
{
    ...
    double PitchIn = (0.5+(double)mp.mSamplePitch * 2) * 1 - (mp.mSamplePitchStepperMod - 0.5 * 0.5)
    ...
    double Level =        (((double)mp.mSampelVolume)*
                          (1 - (oscillator.getSampleLfo() * mp.mSampelVolumeMod)));
...
for (int s = 0; s < numSamples; s++)
{
    ...
    if (mp.mSamplePlayMode == 1 && mp.mEngineMode == false)
    {
        ...
        for (int c = 0; c < MAX_VOICES; c++)
        {
            ...
            if (ArOn == false) {
                rtimeA = 1.0; rtimeR = 1.0; stpoint *= mp.mSpAttack; ;
            }

          

            if (Trigger > 0 && mSampelIndxPoly[pCount] >= enpoint - 2) {    mSampelIndxPoly[pCount] = stpoint;  }

           if (mSampelIndxPoly[c] >= enpoint - 1) { mSampelIndxPoly[c] = enpoint - 1 ; }
        ...
        for (int i = 0; i < nFrames; i++)
        {
            if (mp.mSamplePlayMode == 1)
            {
                ...
                out[i] *= SmootherSynth.Process((rtimeA * rtimeR)* Level) ;
...
void AudioSample::processSynthSampel(const int *trigger, double *Out, int nFrames)
{
...
void AudioSample::processOut(const int *trigger, double *Out, int nFrames)
{
    ...
    {
        processAudioSampel(trigger, Out,nFrames);
    }
    if(mp.mEngineMode==true)
    {
        processSynthSampel(trigger, Out,nFrames);
    }
    for(int s=0;s<nFrames;s++)
    {
   Out[s] *= SmSpVolume.Process(mp.mSampelVolumeM);
