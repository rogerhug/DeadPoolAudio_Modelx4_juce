...
public:
   ...
   SynthOscillator oscillator;
    void processAudioSampel(const int *trigger,double*Out,int nFrames);
    void processSynthSampel(const int *trigger,double*Out,int nFrames);
    void processOut(const int *triggerP,double*Out,int nFrames);
    WDL_ResampleSample mAudioToResample2[SAMPLERATEAUDIO];
    void setWaveSize(int s) { waveSize = s; }
    void setAr(bool o) { ArOn = o;; } 
  
    void loadfile(int size,const int channel)
    {
        numChannels = channel;
       memcpy(mAudioToResample, mAudioToResample2,(size)*sizeof(double));
        waveSize=size;
    }
    const int getTrOff() { return toff; }
