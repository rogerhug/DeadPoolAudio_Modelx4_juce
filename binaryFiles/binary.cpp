...
binaryFiles::binaryFiles()
{
	...
	for (int i = 0; i < WAVESIZEMAX; i++)
	{
	    ...
	    mBuffer[i] = sin(i / (double)WAVESIZEMAX*2*PI);
...
{
	mSampleLenght = mSubChunk2Size / (2 * (mBitsPerSample / 8));
...
{
	fixv = f;
	if (fixv == 0) { sizedev = getSampleLength() / 6; }
	if (fixv == 1) { sizedev = getSampleLength() / 3; }
	if (fixv == 2) { sizedev = getSampleLength(); }
	return sizedev;
}
...
void binaryFiles::setBuffer(int st)
{
	...
	const int ch = 1;
		...
		mBuffer.resize(setSampelFix(fixv) *ch) ;
		mBufferVcfRelease.resize(setSampelFix(fixv)*ch);
		...
		for (int sample = 0; sample < setSampelFix(fixv) *ch; ++sample)
		{
			...
			mBufferVcfRelease[sample] = 1 - (sample / (double)(setSampelFix(fixv) *ch));
